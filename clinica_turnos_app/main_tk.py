...
class FrameLogin(ttk.Frame):
    def __init__(self, master, app: AplicacionTurnos):
        ...
        titulo = ttk.Label(self, text="Ingreso al sistema", font=("Segoe UI", 16, "bold"))
        titulo.grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 14))

        ttk.Label(self, text="Usuario:").grid(row=1, column=0, sticky="w", pady=6)
        self.entry_usuario = ttk.Entry(self, textvariable=self.var_nombre_usuario, width=34)
        self.entry_usuario.grid(row=1, column=1, sticky="w", pady=6)

        ttk.Label(self, text="Contrasena:").grid(row=2, column=0, sticky="w", pady=6)
        self.entry_contrasena = ttk.Entry(self, textvariable=self.var_contrasena, width=34, show="*")
        self.entry_contrasena.grid(row=2, column=1, sticky="w", pady=6)
        ...
        chk.grid(row=3, column=1, sticky="w", pady=(2, 10))

        botones = ttk.Frame(self)
        botones.grid(row=4, column=0, columnspan=2, sticky="w", pady=8)
...
class FramePrincipal(ttk.Frame):
    def __init__(self, master, app: AplicacionTurnos, usuario_actual: dict):
        ...
        ttk.Label(self, text=texto_bienvenida).grid(row=1, column=0, columnspan=5, sticky="w", pady=(0, 14))
        ...
        ttk.Button(self, text="Cambiar mi contrasena", command=self._cambiar_mi_contrasena).grid(
            row=4, column=0, padx=6, pady=(18, 6), sticky="w"
        )

        ttk.Button(self, text="Cerrar sesion", command=self._cerrar_sesion).grid(
            row=4, column=4, padx=6, pady=(18, 6), sticky="e"
        )
...
class FramePacientes(ttk.Frame):
    def __init__(self, master, app: AplicacionTurnos, usuario_actual: dict):
        ...
        self.tabla.grid(row=2, column=0, columnspan=8, sticky="nsew")
...
class FrameUsuarios(ttk.Frame):
    def __init__(self, master, app: AplicacionTurnos, usuario_actual: dict):
        ...
        acciones = ttk.Frame(self)
        acciones.grid(row=2, column=0, columnspan=8, sticky="w", pady=(12, 0))
...
class FrameTurnos(ttk.Frame):
    def __init__(self, master, app, usuario_actual: dict):
        ...
        self.tabla.grid(row=2, column=0, columnspan=10, sticky="nsew")
        for col, texto in [
            ("id", "ID"),
            ("inicio", "Inicio"),
            ("fin", "Fin"),
        ...
        scroll = ttk.Scrollbar(self, orient="vertical", command=self.tabla.yview)
        self.tabla.configure(yscrollcommand=scroll.set)
        scroll.grid(row=2, column=10, sticky="ns")
        acciones = ttk.Frame(self)
...
class VentanaTurno(tk.Toplevel):
    def __init__(self, master, titulo="Turno", fecha_inicial=None, datos_iniciales=None):
        ...
        # Fecha + Hora + Duracion
        ttk.Label(cont, text="Fecha *:").grid(row=2, column=0, sticky="w", pady=6)
        self.cal_fecha = DateEntry(cont, width=12, date_pattern="yyyy-mm-dd")
        self.cal_fecha.grid(row=2, column=1, sticky="w", pady=6)
...
class VentanaCambiarContrasena(tk.Toplevel):
    def __init__(self, master, titulo="Cambiar contrasena", descripcion=""):
        ...
        ttk.Label(cont, text="Repetir contrasena *:").grid(row=2, column=0, sticky="w", pady=6)
        ttk.Entry(cont, textvariable=self.var_contrasena_2, width=34, show="*").grid(row=2, column=1, sticky="w", pady=6)
